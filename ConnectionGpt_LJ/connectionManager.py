...
import json
import os
from statusManager import Daughter as dau

# 전체 대화 내용 저장용 리스트
conversation_ = []

# OpenAI API 키 설정
api_key = 'sk-proj-o2WKJJ7Hxl4iKR6PwEyvT3BlbkFJafAjdBiSVD29QpI0tmTg'
...
def extract_and_save_updated_status(daughter_reply, d_stat):
    #-- 
    ...
    if "**" in daughter_reply:
        ...
        d_stat.name = stat_str_["daughter"]["name"]
        d_stat.sex = stat_str_["daughter"]["sex"]
        d_stat.age = stat_str_["daughter"]["age"]
        d_stat.mbti = stat_str_["daughter"]["mbti"]
        d_stat.hp = stat_str_["daughter"]["hp"]
        d_stat.mp = stat_str_["daughter"]["mp"]
        d_stat.stress = stat_str_["daughter"]["stress"]
        d_stat.fatigue = stat_str_["daughter"]["fatigue"]
        d_stat.E = stat_str_["daughter"]["E"]
        d_stat.I = stat_str_["daughter"]["I"]
        d_stat.S = stat_str_["daughter"]["S"]
        d_stat.N = stat_str_["daughter"]["N"]
        d_stat.T = stat_str_["daughter"]["T"]
        d_stat.F = stat_str_["daughter"]["F"]
        d_stat.J = stat_str_["daughter"]["J"]
        d_stat.P = stat_str_["daughter"]["P"]
        print(type(stat_str_))
        print(d_stat.mood)
        print(d_stat.stress)
        print(d_stat.fatigue)
        #다시 key, value 빼서 set해주기


def ConnectionGpt(d_stat):
    father_status = {
        "father": {
            "name": "Lain",
            "age": 45,
    ...
    #stat_str = extract_and_save_updated_status()
    ...
    while True:
        ...
        try:
            ...
            extract_and_save_updated_status(daughter_reply, d_stat)
