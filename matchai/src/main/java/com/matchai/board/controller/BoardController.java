...
public class BoardController {
    ...
    public ModelAndView mainBoard(ModelAndView mv, HttpServletRequest req) {
        ...
        if (mlbCnt >= 1) {
            ...
            // 경기 없다면 멘트 추가
            mv.addObject("ment", "다음날 MLB 경기는 없습니다.");
        }
        // jsp. 주소 setting
    ...
    public ModelAndView actbaseball(HttpServletRequest req, ModelAndView mv,
                                    @RequestParam(name = "matchcode") String matchcode, @RequestParam(name = "team1") String team1,
                                    @RequestParam(name = "team2") String team2, HttpSession session) {
        System.out.println("MatchCode: " + matchcode);
        // 게임 데이터 조회
        ...
        // 없으면 board에 ai예측 데이터 넣기
        if (count <= 0) {
            String title = actData.get("winname").toString() + " VS " + actData.get("losename").toString();
            String meme = actData.get("winname").toString()
                    + "  "
                    + actData.get("winscore").toString()
                    + "점 "
                    + actData.get("losename").toString()
                    + "  "
                    + actData.get("losescore").toString()
                    + "점으로"
                    +actData.get("winname").toString()
                    +"팀이 승리 하였습니다.";
            actData.put("title", title);
            actData.put("meme", meme);
